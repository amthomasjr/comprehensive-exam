...
\begin{frame}
	...
	\begin{itemize}
		\item Stack all variables $Z_{bsnt}$ in a subblock and pseudo-difference the idiosyncratic components $e_{Zbsnt}$ to produce
			\begin{equation}
				\tilde{\bm Z}_{bst} = \tilde{\bm \Lambda}_{H.bs} {\bm H}_{bst} + {\bm \epsilon}_{Zbst}, \quad {\bm \epsilon}_{Zbst} \sim \mathcal{N} \left( {\bf 0}, {\bm \Sigma}_{Zbs} \right)
			\end{equation}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item The authors implement a Gibbs sampling algorithm to obtain samples from the posterior
		\item 50,000 draws are used as a burn-in
...
\begin{frame}
	...
	\begin{itemize}
		\item Given an i.i.d. dataset ${\bm X} = \left\{ {\bm X}_1, \ldots, {\bm X}_N \right\}$
			\begin{enumerate}
				\item The VBE step:
					\begin{equation}
...
\begin{frame}
	...
	\begin{itemize}
		\item The main goal of the project is to develop a VB framework to handle the three and four-level model from \cite{moenchDynamicHierarchicalFactor2013}
		\item  
	\end{itemize}
\end{frame}
