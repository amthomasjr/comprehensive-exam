...
\begin{frame}
	...
	\textbf{Factor Analysis} (FA) is a method that assumes that the covariance structure of a set of observations can be described in terms of a linear combination of unobservable (latent) variables called factors. 
	\bigskip
...
\begin{frame}
	\frametitle{Factor Analysis}
	FA can be extended to time series data. 
	For the remainder of the presentation we will assume that we are working with time series data.
	\bigskip
	
	For a sample of $T$ time periods we have the following:
	\begin{itemize}
		\item ${\bm F}_t = \left( F_{1t}, \ldots, F_{Kt} \right)^{\top}$ denotes the factors
		\item ${\bm X}_t = \left( X_{1t}, \ldots, X_{Nt} \right)^{\top}$ denotes the observations
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Factor Analysis}
	The measurement equation that relates the observations to the factors is given by
	\begin{equation}
		{\bm X}_t = {\bm \Lambda} {\bm F}_t + {\bm e}_t
	\end{equation}
	where
	\begin{itemize}
		\item ${\bm X}_t = \left( X_{1t}, \ldots, X_{Nt} \right)^{\top}$ denotes a vector of observations
		\item ${\bm \Lambda} = \left[ \lambda_{nk} \right]_{N \times K}$ denotes a matrix of factor loadings that act as weights
		\item ${\bm F}_t = \left( F_{1t}, \ldots, F_{Kt} \right)^{\top}$ denotes a vector of factors
		\item ${\bm e}_t$ is a vector of measurement errors, disturbances, and idiosyncratic (unique) factors
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\end{equation*}
	is a distributed lag matrix polynomial of order $s$ in the lag operator $L$ and
	\begin{equation*}
		L^s {\bm F}_t = {\bm F}_{t-s}.
	\end{equation*}
	The observation equation can be expressed instead as
	\begin{equation*}
		{\bm X}_t = {\bm \Lambda}_0 {\bm F}_t + {\bm \Lambda}_1 {\bm F}_{t-1} + \cdots + {\bm \Lambda}_s {\bm F}_{t-s} + {\bm \epsilon}_t
	\end{equation*}
\end{frame}
...
\begin{frame}
	\frametitle{Hierarchical Dynamic Factor Model}
	From \cite{moenchDynamicHierarchicalFactor2013}, this posterior distribution of the factors and model parameters is estimated via a Gibbs sampler.
	The main steps are:
	\begin{enumerate}
		\item Organize data into blocks 
...
\begin{frame}
	\begin{itemize}
		\item Proceeding in a similar way, we have that
			...
			where
			\begin{align*}
				{\bf P}_{\bm \lambda n}^{-1} &= \E_{{\bm \Sigma}} \left[ \frac{1}{\sigma_n^2} \right] \left( {\bf I}_{K} + \sum_{t = 1}^T \E_{{\bm F}} \left[ {\bm F}_t {\bm F}_t^{\top} \right] \right) \\
				...
				\nu_{\sigma} &= T + \nu_0 \\
				\nu_{\sigma} \tau_n^2 &= \nu_0 \tau_0^2 + \sum_{t = 1}^T \left[ X_{nt}^2 - 2 X_{nt} \E_{{\bm \Lambda}} \left[ {\bm \lambda}_n \right]^{\top} \E_{{\bm F}} \left[ {\bm F}_t \right] + \tr \left[ \E_{{\bm \Lambda}} \left[ {\bm \lambda}_n {\bm \lambda}_n^{\top} \right] \E_{{\bm F}} \left[ {\bm F}_t {\bm F}_t^{\top} \right] \right] \right]
			\end{align*}
	\end{itemize}
\end{frame}
