...
\begin{frame}
	...
	\textbf{Bayesian Inference} can be described by three parts:
	\begin{enumerate}
		\item Build a model based on data $\bm X$ and parameters $\bm \Theta$
			...
			\end{itemize}
		\item Compute the posterior pdf
			\begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
  		\item Consider a model with data $\bm X$ and unknowns $\bm Z$
  		\item The goal is to compute the posterior 
  		\begin{equation} \label{posterior}
  			p \left( {\bm Z} | {\bm X} \right) = \frac{ p \left( {\bm X} | {\bm Z} \right) p \left( {\bm Z} \right) }{ p \left( {\bm X} \right) }
  		\end{equation}
		and report posterior expectations
		\begin{equation} 
			\E \left[ h ( {\bm Z} ) | {\bm X} \right] = \int h ( {\bm Z} ) p \left( {\bm Z} | {\bm X} \right) \,d{\bm Z} \label{posterior.expectation}
		\end{equation}

		\item The term in the denominator is called the evidence 
		\begin{equation} 			
...
\begin{frame}
	\frametitle{Why Variational Bayesian Inference?}
	\begin{itemize}
		\item For most interesting models $\bm Z$ is high-dimensional
		\item (\ref{posterior.expectation}) and (\ref{marginal.evidence}) would require high-dimensional integration
		\item (\ref{posterior}), (\ref{posterior.expectation}), and (\ref{marginal.evidence}) are typically analytically intractable
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Why Variational Bayesian Inference?}
	\begin{enumerate}
		\item The gold standard has been to use MCMC algorithms
		\item MCMC methods allow sampling from intractable distributions
		\item 
	\end{enumerate}
\end{frame}
...
\begin{frame}
	...
	, but takes too long to run. VB methods are faster.
\end{frame}
